...
action_sampler_optimizer = optim.Adam(action_sampler_nn.parameters(), lr=1e-6, weight_decay=0.000001)  # Much lower LR
warm_start_optimizer = optim.Adam(warm_start_nn.parameters(), lr=1e-6, weight_decay=0.000001)  # Much lower LR
...
def save_neural_networks(action_sampler_nn, warm_start_nn, action_sampler_optimizer, warm_start_optimizer, save_dir="saved_models"):
    """
    Save the neural network parameters and optimizer states to files.
    """
    # Create save directory if it doesn't exist
    os.makedirs(save_dir, exist_ok=True)
    
    # Save neural network state dictionaries
    action_sampler_path = os.path.join(save_dir, "action_sampler_nn.pth")
    warm_start_path = os.path.join(save_dir, "warm_start_nn.pth")
    
    torch.save({
        'model_state_dict': action_sampler_nn.state_dict(),
        'optimizer_state_dict': action_sampler_optimizer.state_dict(),
    ...
    torch.save({
        'model_state_dict': warm_start_nn.state_dict(),
        'optimizer_state_dict': warm_start_optimizer.state_dict(),
        'input_size': INPUT_SIZE,
        'hidden_size': 32,
        'num_actions': NUM_TOTAL_ACTIONS
    }, warm_start_path)
    
    print(f"\nNeural networks saved to:")
    print(f"  - Action Sampler: {action_sampler_path}")
    print(f"  - Warm Start: {warm_start_path}")

def load_neural_networks(action_sampler_nn, warm_start_nn, action_sampler_optimizer, warm_start_optimizer, save_dir="saved_models"):
    """
    Load the neural network parameters and optimizer states from files.
    ...
    if not (os.path.exists(action_sampler_path) and os.path.exists(warm_start_path)):
        print(f"Saved model files not found in {save_dir}/. Starting with random weights.")
        return False
    
    try:
        # Load Action Sampler network
        action_sampler_checkpoint = torch.load(action_sampler_path)
        action_sampler_nn.load_state_dict(action_sampler_checkpoint['model_state_dict'])
        action_sampler_optimizer.load_state_dict(action_sampler_checkpoint['optimizer_state_dict'])
        
        # Load Warm Start network
        warm_start_checkpoint = torch.load(warm_start_path)
        warm_start_nn.load_state_dict(warm_start_checkpoint['model_state_dict'])
        warm_start_optimizer.load_state_dict(warm_start_checkpoint['optimizer_state_dict'])
        
        print(f"\nNeural networks loaded successfully from:")
        print(f"  - Action Sampler: {action_sampler_path}")
        print(f"  - Warm Start: {warm_start_path}")
        return True
...
if __name__ == "__main__":
    ...
    parser.add_argument('--load-models', action='store_true', help='Load previously saved neural network models')
    parser.add_argument('--save-models', action='store_true', default=True, help='Save neural network models after training (default: True)')
    parser.add_argument('--model-dir', type=str, default='saved_models', help='Directory to save/load models (default: saved_models)')
    args = parser.parse_args()

    num_iterations = args.iterations
    print(f"Starting MCCFR training for {num_iterations} iterations...")
    
    # Load neural networks if requested
    if args.load_models:
        load_success = load_neural_networks(action_sampler_nn, warm_start_nn, 
                                          action_sampler_optimizer, warm_start_optimizer, 
                                          args.model_dir)
        if load_success:
            print("Continuing training from saved models.")
        else:
            print("Could not load saved models. Starting with random weights.")
    else:
        print("Starting training with random neural network weights.")
    ...
    # Save neural networks after training if requested
    if args.save_models:
        save_neural_networks(action_sampler_nn, warm_start_nn, 
                           action_sampler_optimizer, warm_start_optimizer, 
                           args.model_dir)
    ...
    print(f"\nTraining completed. Models {'saved' if args.save_models else 'not saved'}.")
    if args.save_models:
        print(f"To continue training from these models, use: --load-models")
        print(f"Example: python '{__file__}' --iterations 100000 --load-models")
