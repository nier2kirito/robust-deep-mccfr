...
warm_start_nn = create_network(NETWORK_TYPE).to(device)
...
print(f"Warm Start Network: {warm_start_params:,} parameters")
print(f"Total parameters: {action_sampler_params + warm_start_params:,}")
...
warm_start_optimizer = optim.AdamW(warm_start_nn.parameters(), lr=learning_rate, weight_decay=weight_decay, betas=(0.9, 0.999))
...
WARM_START_MIN_VISITS = 100  # Higher threshold for complex architectures to let MCCFR develop better strategies first
...
def mccfr_outcome_sampling(state: KuhnState, player_card_map: dict[int, Card], inv_reach_prob_sampler: float, training_data: list = None):
    ...
    if training_data is not None and info_node['visits'] >= 20:  # Only collect from well-visited infosets
        # Create target strategy for WARM START network (normalized current strategy)
        ...
        # Create target for ACTION SAMPLER network (regret-based exploration target)
        ...
        # Calculate regret-based sampling probabilities
        regret_scores = np.zeros(NUM_TOTAL_ACTIONS)
        for idx in legal_actions_indices:
            # For actions with positive regrets, use regret magnitude
            # For actions with negative regrets, use small positive value for exploration
            regret_val = current_regrets[idx]
            if regret_val > 0:
                # Positive regret - sample more frequently (square to emphasize high regrets)
                regret_scores[idx] = regret_val ** 1.5  # Power between 1-2 for good balance
            else:
                # Negative or zero regret - still allow some exploration
                regret_scores[idx] = 0.1  # Small constant for minimal exploration
...
def train_neural_networks_on_batch(batch_data):
    """Train neural networks on a batch of collected data with different targets."""
    if not batch_data:
        return 0.0, 0.0
    
    # Unpack batch data - now includes separate targets for each network
    ...
    accumulation_steps = 2 if NETWORK_TYPE in ['feature_attention', 'hybrid_advanced'] else 1
    ...
    for i in range(accumulation_steps):
        ...
        if NETWORK_TYPE in ['feature_attention', 'hybrid_advanced']:
    ...
    for i in range(accumulation_steps):
        ...
        warm_start_loss = warm_start_loss / accumulation_steps
        warm_start_loss.backward()
        total_warm_start_loss += warm_start_loss.item()
    
    # Gradient clipping
    if NETWORK_TYPE in ['feature_attention', 'hybrid_advanced']:
        torch.nn.utils.clip_grad_norm_(warm_start_nn.parameters(), max_norm=0.5)
    else:
